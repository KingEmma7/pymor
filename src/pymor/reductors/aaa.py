...
class PAAAReductor(BasicObject):
    ...
    :meth:`sample_transfer_function` to generate data from a model beforehand.
    ...
    def __init__(self, sampling_values, samples, enforce_real=True, nsp_tol=1e-16, post_process=True,
        ...
        if self.samples.ndim not in (self.num_vars, self.num_vars + 2) \
            ...
            raise ValueError('samples must contain data on the sampling grid; use sample_transfer_function for models.')
        ...
        while any(len(i) < mi for i, mi in zip(self.itpl_part, max_itpl, strict=True)):
            ...
            for i in range(self.num_vars):
                if greedy_idx[i] not in self.itpl_part[i] and len(self.itpl_part[i]) < max_itpl[i]:
                    ...
                    if i == 0 and self.enforce_real and np.imag(svs[i][greedy_idx[i]]) != 0:
